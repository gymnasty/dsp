...
export const ItemList = () => {
  ...
  const CategoryGrid = ({ category }: { category: string }) => {
    const unsortedItems = Object.values(ITEMS).filter(item => item.type === ITEM_TYPES.BUILDING && item.category === category);
    if (unsortedItems.length === 0) return null;

    // Sort items based on the predefined order
    const order = BUILDING_ORDER[category] || [];
    ...
    // Create a simple grid: 14 items per row
    const columns = 14;
    const grid: (string | null)[][] = [];
    for (let i = 0; i < items.length; i += columns) {
      const row = items.slice(i, i + columns).map(item => item.id);
      while (row.length < columns) row.push(null);
      grid.push(row);
    }

    return (
      <div className="inline-block border border-slate-300 rounded overflow-hidden shadow-sm mr-4 mb-8 align-top">
        <div className="bg-slate-100 px-4 py-2 border-b border-slate-300">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t(`categories.${category}`)}</span>
        </div>
        <div className="bg-white p-1">
          <table className="border-collapse">
            <tbody>
