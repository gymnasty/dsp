  ...
  if (!item) {
    return (
      <div className="max-w-6xl mx-auto py-20 text-center">
        <h2 className="text-2xl font-bold text-slate-400">{t('itemDetail.itemNotFound')}</h2>
        <Link to="/items" className="text-blue-600 hover:underline mt-4 inline-block">{t('itemDetail.returnToDirectory')}</Link>
      </div>
    );
  }
          ...
          {itemRecipes.length > 1 && (
            <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl">
              {itemRecipes.map((r, index) => (
                ...
                >
                  {t('itemDetail.recipe')} {index + 1}: {t(`facilities.${r.producedIn}`)}
                </button>
              ))}
            </div>
          )}
            ...
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              {recipe && (
                <div className="flex gap-4">
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{t('itemDetail.cycleTime')}</span>
            ...
            <div className="p-8">
              {recipe ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-grow">
                            ...
                            <div className="text-[10px] font-bold text-slate-400 uppercase">{t(`categories.${ingItem?.category}`)}</div>
                          </div>
                          <div className="text-lg font-black text-blue-600 font-mono text-right">
                            <div className="text-[10px] text-slate-400 uppercase tracking-tighter">x{ing.count} {t('itemDetail.base')}</div>
                  ...
                  <div className="bg-slate-900 rounded-2xl p-6 flex flex-col gap-4 text-white">
                    ...
                    {recipe.extraOutputs && recipe.extraOutputs.length > 0 && (
                      <div className="pt-4 border-t border-white/10 space-y-3">
                        <div className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{t('itemDetail.byproducts')}</div>
                ...
                <div className="py-12 flex flex-col items-center justify-center text-slate-400 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  ...
                  <p className="font-bold">{t('itemDetail.rawMaterial')}</p>
                  <p className="text-xs">{t('itemDetail.rawMaterialDesc')}</p>
                </div>
              )}
            </div>
          </div>
              ...
              {usedIn.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {usedIn.map((r) => {
                    ...
                    const outputName = getItemName(outputItem);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-grow">
                          ...
                          <div className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">{t(`categories.${outputItem?.category}`)}</div>
                        </div>
                        <div className="text-right">
                          <span className="text-xs font-bold text-slate-400 uppercase block">{t('itemDetail.requires')}</span>
